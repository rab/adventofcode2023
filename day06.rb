...
# Island!" That must be where the sand comes from! Best of all, the boat races are starting in
# just a few minutes.
#
...
#   the button. The boat can't move until you let you of the button. Please make sure you let go of
...
t,d=input.lines(chomp: true)
_time, *times = t.split(' ')
times = times.map(&:to_i)
_distance, *distances = d.split(' ')
distances = distances.map(&:to_i)

wins = Array.new(times.size, 0)
times.zip(distances).each.with_index do |(time,distance),race|
  time.times do |hold|
...
big_t = times.join.to_i
big_d = distances.join.to_i
big_t.times do |hold|
  if (big_t - hold) * hold > big_d
    part2 += 1
  end
end
