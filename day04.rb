...
# won. Including the original set of scratchcards, how many total scratchcards do you end up with?
...
input.each_line(chomp: true) do |line|
  ...
  _c, l1, l2 = line.match(/\ACard *(\d+):(.*?)\|(.*?)\z/).captures
  pp _c, l1, l2 if debugging
  winners = l1.scan(/\d+/).map(&:to_i)
  numbers = l2.scan(/\d+/).map(&:to_i)
  puts "Card #{_c}:#{winners.map{|w| '%3d'%[w]}} |#{numbers.map{|n| '%3d'%[n]}}" if debugging
  wins = numbers.select { winners.include? _1 }
  cards[_c.to_i] = { count: 1, wins: wins.size }
  part1 += 2 ** (wins.size - 1) if wins.any?
end
...
cards.keys.each do |card|
  cards[card][:wins].times do |idx|
    cards[card+1+idx][:count] += cards[card][:count]
  end
end
part2 = cards.values.map{_1[:count]}.sum
