...
class Game
  include Enumerable
  attr_accessor :game, :pulls, :bag

  def initialize(line)
    g, l = line.split(": ",2)
    @game = g.sub(/\AGame (\d+)\z/, "\\1").to_i
    @bag = Hash.new(0)
    @pulls = []
    l.split(';').each do |pull|
      this_pull = Hash.new(0)
      pull.scan(/ *(\d+) (red|green|blue)(?:, *)?/).each do |cnt,color|
        cnt = cnt.to_i
        this_pull[color] += cnt
        if cnt > @bag[color]
          @bag[color] = cnt
...
input.each_line(chomp: true) do |line|
  ...
  game = Game.new(line)
  part2 += game.power_set
  next if game.pulls.any? {|pull| max_1.detect {|color,cnt| pull[color] > cnt } }
  part1 += game.game
end
