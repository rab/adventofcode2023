...
class Mapping
  ...
  def initialize(from, to)      # e.g., if "seed-to-soil", from is "seed" and to is "soil"
    @from = from
    @to = to
    @maps = []
  end

  def add(dest,source,range)
    @maps << [(source...(source+range)), (dest...(dest+range))]
  end

  def [](input)
    @maps.each do |domain,range|
      next unless domain.cover? input
      return range.begin + (input - domain.begin)
    end
    input
  end
...
def chain(maps,input)
  # print "\nSeed #{input}"
  maps.reduce(input) {|res,map|
    # print ", #{map.to} #{map[res]}"
    map[res]
  }
end
...
input.each_line("", chomp: true) do |block|
  puts block if debugging
  case block
  when /\Aseeds: (.*)\z/
    seeds = $1.split.map(&:to_i)
    seed2 = seeds.each_slice(2).map{|start,range| (start...(start+range)) }
  when /\A(\w+)-to-(\w+) map:/
    map = Mapping.new($1,$2)
    block.each_line.with_index do |line,idx|
      next if idx.zero?
...
# puts seeds.inspect, mappings

part1 = seeds.map{chain(mappings,_1)}.min
...
part2 = seed2.map{|r| r.map{chain(mappings,_1)}.min }.min
