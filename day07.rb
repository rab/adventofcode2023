...
class Hand
  CARDS = "AKQJT98765432".chars.zip((2..14).to_a.reverse).to_h
  WILDS = "AKQT98765432J".chars.zip((2..14).to_a.reverse).to_h

  def initialize(line)
    deal, @bet = line.split(' ')
    @bet = @bet.to_i
    @cards = deal.chars
    @sort = [ self.rank, *(@cards.map{CARDS[_1]})]
    @wild = [ self.wrank, *(@cards.map{WILDS[_1]})]
  end
  ...
  def rank
    ...
    { "5"     => 7,
      ...
      "2111"  => 2,
      "11111" => 1,
    }[counts.values.sort.reverse.join]
  end

  def wrank
    counts = Hash.new(0)
    jokers, regs = @cards.partition { _1 == "J" }
    if regs.empty?
      return 7                  # must be "JJJJJ"
    else
      regs.each {counts[_1] += 1}
      temp = counts.values.sort.reverse
      temp[0] += jokers.size
      regs.each {counts[_1] += 1}
      { "5"     => 7,
        "41"    => 6,
...
hands.sort_by(&:sort).each.with_index(1) {|hand,idx| part1 += hand.bet * idx }
...
part2 = 0
hands.sort_by(&:wild).each.with_index(1) {|hand,idx| part2 += hand.bet * idx }
